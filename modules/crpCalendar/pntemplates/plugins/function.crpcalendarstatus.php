...
function smarty_function_crpcalendarstatus($params, &$smarty)
{
	...
	{		
		if (SecurityUtil::checkPermission('crpCalendar::', '::', ACCESS_ADD) && ($params['status']=='A' || $params['status']=='P'))
	  	$statusimage .= '<a href="'.pnModUrl('crpCalendar','admin','change_status', array('eventid' => $params['eventid'], 'obj_status'=>$params['status'])).'" title="'._CRPCALENDAR_CHANGE_STATUS.'">'."\n";
		else
			$statusimage .= '';
	  ...
	  elseif ($params['status']=='P')
	    $statusimage .= '<img id="eventstatus_'.$params['eventid'].'" src="modules/crpCalendar/pnimages/yellow_dot.gif" alt="'._CRPCALENDAR_PENDING.'" title="'._CRPCALENDAR_CHANGE_STATUS.'"/>'."\n</a>\n";
		else
