...
class crpCalendarDAO 
{
	 ...
	 * @return array element list
	 */
	function formList($startnum=1, $category=null, $clear=false, $ignoreml=true, 
											$modvars=array(), $mainCat, $active=null, $interval=null, 
											$sortOrder='DESC')
	{
		if (!is_numeric($startnum) ||
    ...
    $permFilter = array(array('realm'           => 0,
		...
		$columnArray = array('eventid','title');
    ...
    if ($objArray === false) {
		...
		foreach ($objArray as $kObj => $vObj)
		{
			$formArray[] = array('id' => $vObj['eventid'], 'name' => $vObj['title']); 
		}
    ...
    $columnArray = array('eventid','cr_uid');
    ...
    $columnArray = array('eventid',''.$dateType.'');
    ...
    if (!$object) 
    {
      ...
      return false;
		...
		if(isset($inputValues['event_document']) && ($inputValues['event_document']['error'] == UPLOAD_ERR_OK))
		{
			...
			if($id_document == '-1')	return false;
		}
		
    // Let any other modules know we have created an item
    pnModCallHooks('item', 'create', $inputValues['eventid'], array('module' => 'crpCalendar'));
    ...
    if (!$object) 
    {
      ...
      return false;
		...
		if(isset($inputValues['image']) && $inputValues['image']['id'])
		{
			...
			$inputValues['event_image'] = array('eventid' => $object['eventid'],
		...
		if(isset($inputValues['document']) && $inputValues['document']['id'])
		{
			...
			$inputValues['event_document'] = array('eventid' => $object['eventid'],
	...
	{		
		...
		{
			$partecipation = array('uid' => $uid, 'eventid' => $eventid);
    ...
    $columnArray = array('id','eventid','document_type','name','content_type','size');
    if ($load_binary)
    ...
    $columnArray = array('id','eventid','document_type','name','content_type','size','binary_data');
