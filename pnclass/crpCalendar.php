...
class crpCalendar
{
	...
	function manageEvents()
	{
		...
		foreach ($items as $key => $item)
		{
			...
			if (SecurityUtil :: checkPermission('crpCalendar::', "::", ACCESS_ADD)
					|| (SecurityUtil :: checkPermission('crpCalendar::', "::", ACCESS_EDIT) && $this->isAuthor($item['eventid'])))
			{
				$options[] = array (
					'url' => pnModURL('crpCalendar',
					'admin',
		...
		return $this->ui->adminList($rows, $navigationValues['category'],
																$navigationValues['mainCat'], $navigationValues['modvars'], $navigationValues['active']);
	}
	...
	function listMonthEvents()
	{
		...
		return $this->ui->userMonthList($items, $days, $daysexpanded, $navigationValues['t'], $date,
	...
	function listWeekEvents()
	{
		...
		$days=array(DateUtil::getDatetime($navigationValues['t']));
		...
		return $this->ui->userWeekList($items, $days, $daysexpanded, $navigationValues['t'], $date,
																		$navigationValues['startDate'], $navigationValues['endDate'], $today,
																		$navigationValues['category'], $navigationValues['mainCat'], $navigationValues['modvars']);
	}
    ...
    if ($item == false || ($item['obj_status'] == 'P' && !SecurityUtil::checkPermission('crpCalendar::', '::', ACCESS_EDIT))) {
		...
		pnSessionSetVar('crpCalendar_return_url', pnModURL('crpCalendar', 'user', 'display',array('eventid'=>$item['eventid'])));
    ...
    if (!SecurityUtil::confirmAuthKey()) 
		...
		// Security check
    if (!SecurityUtil::checkPermission('crpCalendar::', "::", ACCESS_ADD)
    		&&  !(SecurityUtil :: checkPermission('crpCalendar::', "::", ACCESS_MODERATE) && $this->isAuthor($inputValues['eventid']))) {
        return LogUtil::registerPermissionError();
    }
    ...
    {
    	LogUtil::registerError (_CRPCALENDAR_INVALID_DATE);
    	return pnRedirect(pnModUrl('crpCalendar','admin','modify', array('eventid' => $inputValues['eventid'])));
    }
    ...
    {
      // Error
      return pnRedirect(pnModUrl('crpCalendar',$returnType,'modify', array('eventid' => $inputValues['eventid'])));
    }
    ...
    // change values
    $inputValues['event'] = array('title' 		=> _CRPCALENDAR_CLONE_TITLE.' '.$item['title'],
            											'urltitle' 	=> null, 
            											'location' 	=> $item['location'],
            											'url' 			=> $item['url'],
            											'contact' 	=> $item['contact'],
            											'organiser' => $item['organiser'], 
            											'__CATEGORIES__' => array('Main' => $item['__CATEGORIES__']['Main']['id']),
    ...
    $inputValues['image'] = array('id' => $item['image']['id']);
    $inputValues['document'] = array('id' => $item['document']['id']);
	...
	function deleteFile()
	{
		...
		// Security check
    if (!SecurityUtil::checkPermission('crpCalendar::', "::", ACCESS_ADD)
    		&&  !(SecurityUtil :: checkPermission('crpCalendar::', "::", ACCESS_EDIT) && $this->isAuthor($eventid))) {
        return LogUtil::registerPermissionError();
    }
    ...
    pnModCallHooks('module','updateconfig','crpCalendar', array('module' => 'crpCalendar'));
	...
	function collectNavigationFromInput()
	{
		...
		$data = compact('startnum', 'category', 'active', 'clear', 
										'ignoreml', 'mainCat', 'cats', 'modvars', 
										'sortOrder', 't');
	...
	function getICal()
	{
		...
		$selfurl = pnModUrl('crpCalendar','user','display',array('eventid'=>$eventid));
	...
	function getThumbnail()
	{
		...
		if(!SecurityUtil::checkPermission('crpCalendar::', '::', ACCESS_READ)) pnShutDown();
